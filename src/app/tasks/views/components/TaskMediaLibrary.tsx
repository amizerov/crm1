...
import { getTaskImages, deleteTaskImage, TaskImage } from '../../actions/taskMediaLibrary';

interface TaskMediaLibraryProps {
  projectId: number;
  taskId: number;
...
export default function TaskMediaLibrary({ projectId, taskId, onImageSelect, onClose }: TaskMediaLibraryProps) {
  ...
  const loadImages = async () => {
    try {
      setIsLoading(true);
      const result = await getTaskImages(projectId, taskId);
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const result = await uploadTaskImage(projectId, taskId, formData);
  ...
  const handleDeleteImage = async (imagePath: string) => {
    ...
    try {
      const result = await deleteTaskImage(projectId, taskId, imagePath);
